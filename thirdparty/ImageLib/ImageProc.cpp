...
#define PRINT_DEBUG(expr) std::cout << "DEBUG>> " << #expr << " = " << (expr) << std::endl

template <class T1, class T2>
void TypeConvertTyped(const T1* src, T2* dst, int n,
...
// template <class T1, class T2>
// void
// TypeConvertLine(const T1* src, T2* dst,
//                 int width, float scale, float offset, float minVal, float maxVal)
// {
...
template <class T1, class T2>
void 
TypeConvert(const CImageOf<T1>& src, CImageOf<T2>& dst)
{
    ...
    double scale = 1;
    if(typeid(T1) == typeid(T2)) scale = 1;
    else if(typeid(T1) == typeid(uchar) && typeid(T2) == typeid(float)) scale = 1/255.0;
    else if(typeid(T1) == typeid(float) && typeid(T2) == typeid(uchar)) scale = 255.0;
    else {
        throw CError("Cannot handle combination of types");
    }

    for(int y = 0; y < sShape.height; y++) {
        TypeConvertTyped(&src.Pixel(0, y, 0), (T2*)dst.PixelAddress(0, y, 0), sShape.width * sShape.nBands, scale, 0, minVal, maxVal);
    }
#if 0
    // Process each row
    for (int y = 0; y < sShape.height; y++)
...
template void TypeConvert<>(const CImageOf<unsigned char>& src, CImageOf<float>& dst);
template void TypeConvert<>(const CImageOf<float>& src, CImageOf<unsigned char>& dst);
template void TypeConvert<>(const CImageOf<float>& src, CImageOf<float>& dst);
template void TypeConvert<>(const CImageOf<unsigned char>& src, CImageOf<unsigned char>& dst);
