...
std::ostream& operator<<(std::ostream& s, const CShape& shape);
...
public:
    ...
    const type_info& PixType(void) const  { return *m_pTI; }
    int BandSize(void) const              { return m_bandSize; }
...
public:
    ...
    T MinVal(void) const;     // minimum allowable value (for clipping)
    T MaxVal(void) const;     // maximum allowable value (for clipping)

    void getRangeOfValues(T& minVal, T& maxVal) const; // Min and max values for current image
